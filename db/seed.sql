...
Большинство бенчмарков на эту границу не выходят. LIMIT — выходит. Пятьдесят тысяч документов, тысяча запросов, предложения типа «Jon likes apples». Лучшие эмбеддинг-модели падают ниже 20% Recall@100 при 4096 измерениях. BM25 по той же метрике выдаёт 93,6%. Проблема в геометрии одного вектора: обученный эмбеддинг представляет конечное число top-$k$ наборов, и когда запрос выпадает из этого набора, ничего дальше по цепочке ситуацию не исправляет.
...
Ошибки складываются. Hard-negative документы — похожие-но-не-те, которые хороший ретривер и должен находить — ухудшают итоговую точность. А случайные несвязанные документы, как показывает [arXiv:2401.14887](https://arxiv.org/abs/2401.14887), её улучшают — максимум на 35%. Починка одной проблемы обычно вскрывает следующую.
...
Поиск тут вторичен. Karpathy прямо пишет, что большинству проектов (около сотни источников, несколько сотен страниц) хватает `index.md` — простого markdown-каталога, который LLM ведёт сама, — и без эмбеддинг-стека можно обойтись. Векторы можно добавить, когда каталог перестанет справляться. До тех пор они не нужны.

Гист строится на аналогии с **компиляцией**. Источники на входе, вики на выходе, каждый ингест — инкрементальная сборка поверх предыдущего артефакта. Один источник обычно затрагивает несколько страниц. Через несколько ингестов каждая страница вбирает данные из разных источников, и результат перестаёт быть похожим на пересказ отдельного документа. Генератор работает по скомпилированным страницам, а не по сырому тексту. Главную мысль гиста несут два фрагмента: «The human''s job is to curate sources, direct the analysis, ask good questions, and think about what it all means. The LLM''s job is everything else» и «Humans abandon wikis because the maintenance burden grows faster than the value. LLMs don''t get bored, don''t forget to update a cross-reference, and can touch 15 files in one pass».
