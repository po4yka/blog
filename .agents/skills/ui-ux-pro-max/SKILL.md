...
> **Project Override:** This project is a personal developer portfolio/blog (po4yka.dev) using Astro 6 + React 19 + Tailwind CSS 4. The design system is fully defined in `DESIGN.md` and `src/styles/theme.css`. Read those files before making any design decisions. See also `.agents/skills/uiux-context/SKILL.md` and `.agents/skills/taste-context/SKILL.md` for project overrides that take precedence over this skill's generic recommendations.
...
> **Warning:** Do NOT use `--design-system` to generate a new design system for this project. The design system is already complete in `DESIGN.md` and `src/styles/theme.css`. Use `--domain` searches for supplementary guidance only.

The `--design-system` flag generates comprehensive recommendations with reasoning:
...
Get Astro implementation-specific best practices:

```bash
python3 .agents/skills/ui-ux-pro-max/scripts/search.py "<keyword>" --stack astro
```
...
| `html-tailwind` | Tailwind CSS utilities, responsive patterns |
...
Scope notice: The rules below apply to web UI. Mobile-app-specific patterns (iOS HIG, Material) are included as reference for cross-platform thinking.
