...
# Background style prompts — adapted for this project's Swiss + operator-console design language.
# The original 12-style set has been trimmed to the 6 styles compatible with DESIGN.md.
# Banned (do NOT re-add): fluid, analog_liquid, led_matrix, iridescent, morning, ui_container —
# each introduces chromatic accent, iridescence, glow, or gradients that violate the design system.
BACKGROUND_STYLES = {
    "void": """THE VOID
Absolute black (#000000) background with extremely fine silver/white high-contrast micro noise.
Cold, sharp electronic film grain texture. Minimal atmosphere light - only a faint icy white glow
at one extreme corner, like distant starlight at the edge of the universe.
No chromatic tint. Pure neutral greyscale only.""",

    "frosted": """FROSTED HORIZON
Deep titanium gray or midnight slate gray base, not pure black. Organic film-like dust noise
texture, resembling unpolished rough metal or stone surface. Large area but extremely low
saturation neutral-gray light halo (no blue cast), edges completely dissolved like mist.
Stay strictly in neutral greys.""",

    "spotlight": """STUDIO SPOTLIGHT
Extremely dark warm carbon gray base. Slightly larger grain simulating low-light camera
photography, like paper print grain in weak light. Single-side softbox or spotlight creating
natural vignette, editorial magazine quality with professional photography feel.
Strictly monochrome — no chromatic cast.""",

    "editorial": """EDITORIAL PAPER
Off-white, alabaster, or pearl white base (not pure white). High-grade watercolor or rough art
paper texture suggesting physical paper tactile quality. Natural light diffuse reflection with
slight warm gray vignette in corners. Humanistic, independent magazine aesthetic.
No chromatic accent — only warm neutral paper tones.""",

    "clinical": """CLINICAL STUDIO
Pure white or extremely light cold gray base. High-frequency sharp cold-toned digital micro noise
with enhanced sharpness. Pure light/shadow structure - large softbox from top/side creating smooth
gray-white gradient. Sterile space with geometric order, creating 3D depth in 2D presentation.
Strictly greyscale — no color.""",

    "swiss_flat": """SWISS FLAT
100% pure solid color background. Choose ONE of the project tokens only: deep graphite (#141416),
warm paper (#f5f3ee), ink (#101012), or eggshell (#e9e8e4) — no vintage greens, burgundies,
or navies. Absolutely no gradients, no noise, no effects. Pure graphic design with zero tricks.
Just perfect neutral color and form. Classic Swiss International Style with absolute flatness."""
}
...
def generate_showcase_image(
    ...
    # Dark backgrounds use white logo (--emphasis on dark); light backgrounds use black logo.
    # swiss_flat spans both — decide by the target color the user picks at generation time;
    # for simplicity we assume the darker two project tokens (#141416, #101012) and fall back
    # to dark-background treatment. Override by editing the prompt if rendering on paper/eggshell.
    dark_styles = ["void", "frosted", "spotlight", "swiss_flat"]
    light_styles = ["editorial", "clinical"]
...
def main():
    ...
    parser.add_argument("--style",
                       ...
                       default="swiss_flat",
                       help="Background style (default: swiss_flat — on-brand for this project)")
