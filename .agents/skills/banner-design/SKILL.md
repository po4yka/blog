...
Collect by asking the user directly, or with `request_user_input` when available:
...
1. **Create HTML/CSS banner** using `frontend-design` skill
   ...
   - Inject brand context via `.agents/skills/brand-system/scripts/inject-brand-context.cjs`
...
- **Brand**: always inject via `.agents/skills/brand-system/scripts/inject-brand-context.cjs`
