...
## Step 5: Data-layer accessors (only if needed)

Add `getAll<Entities>` / `upsert<Entity>` / `delete<Entity>` wrappers to `src/lib/db.ts` only when non-route code needs them, following the existing `getAllPosts` pattern there.
...
export const PUT = <entity_plural>.routes.update;
...
- [ ] API routes: `index.ts` (GET/POST) + `[id].ts` (PUT/DELETE) -- re-export from collection
...
- [ ] Migration added under `db/migrations/` for the new table
