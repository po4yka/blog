...
For automated triage: `desloppify plan triage --run-stages --runner codex`. Options: `--only-stages`, `--dry-run`, `--stage-timeout-seconds`.
...
Use Codex-native review and triage flows.

### Review workflow

Prefer the built-in Codex runner:

1. **Prepare and run**: `desloppify review --run-batches --runner codex --parallel --scan-after-import`
2. **Fallback manual batching**:
   - Run `desloppify review --prepare` to generate `query.json` and `.desloppify/review_packet_blind.json`
   - Split dimensions into 3-4 themed batches
   - Use `spawn_agent` only if the user explicitly requests delegated or parallel review work
   - Give each agent the codebase path, assigned dimensions, and the blind packet path
   - Require each agent to score from code evidence only and write results to `results/batch-N.raw.txt`
   - Merge the outputs and import them with `desloppify review --import merged.json --manual-override --scan-after-import`

### Subagent rules

1. Each agent must be context-isolated — do not pass score targets or prior review history.
2. Agents must consume `.desloppify/review_packet_blind.json` rather than the full `query.json` to avoid score anchoring.

### Triage workflow

Prefer the built-in Codex runner:
1. `desloppify plan triage --run-stages --runner codex`
2. If manual staging is needed, work stage-by-stage:
   - Get prompt: `desloppify plan triage --stage-prompt <stage>`
   - Run the stage locally or with a spawned agent
   - Verify: `desloppify plan triage`
