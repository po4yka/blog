...
1. `docs/Guidelines.md` - Human-readable brand doc
...
Ask the user directly, or use `request_user_input` when available, to collect:
...
node .agents/skills/brand-system/scripts/inject-brand-context.cjs --json | head -30
...
- Secondary: [name] ([hex])
- Accent: [name] ([hex])
- Files updated: Guidelines.md, design-tokens.json, design-tokens.css

## Files Modified

| File | Purpose |
|------|---------|
| `docs/Guidelines.md` | Human-readable brand documentation |
...
- **Always sync all three files** - Never update just Guidelines.md alone
