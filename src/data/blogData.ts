...
You can feel the ceiling before you can measure it. At a few hundred thousand documents, a well-tuned vector index starts returning near-misses on queries it answered perfectly at a few thousand. You add a reranker and the top-1 moves back. You add hybrid search and the long tail gets better. Then you keep growing, and the failures come back -- the same kind, just harder to reproduce. Most teams read this as tuning work. Weller et al. (ICLR 2026) offer a different explanation: a single-vector retriever has a representational ceiling, and past it no amount of downstream cleverness compensates.
...
The failures that follow cluster into three layers. Geometry (the ceiling above) is structural -- no tuning moves it. Chunking and context utilisation are budget problems -- fixable with more careful preprocessing, but the budgets themselves are shrinking. Attention and hard negatives are generator failures -- fixable with better prompts, until the prompts stop mattering. Only the geometry layer has a hard floor. Everything else is a trade.
...
RAG is the correct first answer to "how do I ground an LLM in my data." For a prototype, for a proof of concept, for the first integration -- it works, it's cheap, it buys you the shape of the problem. The mistake isn't using RAG; the mistake is scaling it. Treating the embed-search-read-generate loop as an architecture -- something that can carry a product -- is where it breaks, because the loop pays the full retrieval cost on every question and caps out where the geometry does.
...
The three systems in Part 2 all commit to the wiki shape: compile the corpus into markdown, let the human read the artefact. Others solve the same problem from different angles, and what they keep as the artefact says as much as their mechanics. Four families are worth naming.
...
Потолок чувствуется раньше, чем измеряется. На нескольких сотнях тысяч документов хорошо настроенный векторный индекс начинает возвращать околопопадания на запросах, на которые идеально отвечал при нескольких тысячах. Добавляешь реранкер -- top-1 возвращается. Добавляешь гибридный поиск -- длинный хвост становится лучше. Продолжаешь расти -- и отказы возвращаются, те же по характеру, но более капризные к воспроизведению. Большинство команд читает это как работу по тюнингу. Weller и соавторы (ICLR 2026) дают другое объяснение: у одновекторного ретривера есть репрезентативный потолок, и выше него никакая умная надстройка в конце пайплайна не помогает.
...
Статья в пяти частях. Часть 1 проходит по местам, где плоский RAG ломается, со ссылками на исследования. Часть 2 -- архитектурная альтернатива, LLM Wiki Карпати, и почему она переформулирует пайплайн как компиляцию, а не как поиск. Часть 3 -- другие системы, делающие тот же манёвр в разных формах (агентные memory-системы, граф-RAG, HippoRAG). Часть 4 -- где вики оказывается неправильным ответом. Часть 5 -- что ломается первым, когда её действительно строишь, из опыта сборки 197-коммитного хранилища для команды AI-инфраструктуры.

Weller и соавторы (ICLR 2026) записывают потолок как неравенство. Для корпуса из $n$ документов, top-$k$ запросов и зазора score $\\gamma$ размерность эмбеддинга $d$ должна удовлетворять условию $d \\geq \\frac{\\log \\binom{n}{k}}{\\log(1 + 1/\\gamma)}$. Ниже этой границы часть top-$k$ комбинаций просто нельзя представить в векторном пространстве. Граница работает в одну сторону: если размерность эмбеддинга ниже порога для данного размера корпуса, никакой реранкер, гибридный поиск или prompt engineering не вытащит пропущенные комбинации. Геометрия уже неправильная.

Дальнейшие режимы сбоя группируются в три слоя. Геометрия (потолок выше) -- структурный слой, его тюнингом не сдвинуть. Чанкинг и утилизация контекста -- бюджетные задачи, решаемые аккуратной предобработкой, но сами бюджеты усыхают. Внимание и hard-negatives -- провалы генератора, решаемые лучшими промптами, пока промпты вообще ещё работают. Только у геометрии жёсткий пол. Всё остальное -- компромиссы.
...
От RAG отказываться не надо. RAG действительно правильный первый ответ на вопрос «как прицепить LLM к своим данным». Для прототипа, для proof-of-concept, для первой интеграции -- он работает, стоит недорого и даёт форму задачи. Ошибка не в том, чтобы использовать RAG; ошибка в том, чтобы его масштабировать. Когда к циклу эмбеддинг-поиск-чтение-генерация начинают относиться как к архитектуре -- к чему-то, на чём можно держать продукт, -- он ломается именно там, потому что платит полную стоимость извлечения на каждый вопрос и упирается в тот же потолок, где упирается геометрия.
...
Все эти режимы сбоя делят одно допущение: корпус остаётся сырым, и модель каждый раз заново выводит своё понимание его на каждом запросе. Альтернатива -- скомпилировать корпус один раз, в артефакт, который модель читает напрямую, и заплатить стоимость на ингесте, а не на запросе. Karpathy в апреле 2026 года опубликовал гист, где назвал эту вторую ветку **LLM Wiki**. Дальше -- что это значит архитектурно и три очень разные системы, которые уже её отгрузили.
