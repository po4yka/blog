...
The first pre-commit hook I wrote for the vault blocked every commit I actually wanted to make. The rule was fine; the binding site was wrong, and it took four error-output reads before I saw it. I spent three weeks building that vault for an AI infrastructure team, and the tooling decisions surfaced their reasons only after they broke. What eventually became a 197-commit knowledge base started as a reaction against the retrieval stack I had been shipping before, and that stack has a ceiling.
...
Hard-negative documents -- the near-miss cases a good retriever is designed to surface -- degrade end-to-end accuracy. Random unrelated documents improve it by roughly 35% in OP-RAG experiments ([arXiv:2409.01666](https://arxiv.org/abs/2409.01666)). Fixing one failure mode surfaces the next.

Contextual Retrieval rewrites chunks with surrounding context before embedding: 49% failure reduction, 67% with a reranker. GraphRAG pushes multi-hop accuracy from 23% to 87% at indexing costs one to two orders of magnitude higher, and LazyGraphRAG recovers most of the quality at a fraction of the cost. Self-RAG and CRAG move retrieval policy into the model itself. None of them change the basic loop: embed, search, read, generate, repeat. Once per question, paying the full cost again.
...
I don't have a good way to tell whether the wiki is improving. Add-rate is easy to measure and doesn't mean what I'd like it to mean. The public benchmarks (LongMemEval [arXiv:2410.10813](https://arxiv.org/abs/2410.10813), LOCOMO [arXiv:2504.19413](https://arxiv.org/abs/2504.19413), DMR [arXiv:2501.13956](https://arxiv.org/abs/2501.13956)) test single-turn factual retrieval and little else, and Zep's own DMR paper admits the questions are ambiguous enough that a high score can reflect LLM inference skill rather than memory fidelity. No published benchmark measures time-travel consistency: the same question asked at different points as the corpus evolves. Summarisation-fidelity metrics like F-Fidelity and FaithEval exist. No memory system paper reports using them.
...
Первый pre-commit hook, который я написал для хранилища, блокировал любую попытку закоммитить. Правило было верное, привязка нет, и я понял это только после четвёртого чтения вывода об ошибке. Три недели я строил это хранилище для команды AI-инфраструктуры, и тулинг начал объяснять свои решения только после того, как сломался. База знаний из 197 коммитов выросла из реакции на стек поиска, который я использовал до этого. У этого стека есть потолок.

Weller и соавторы (ICLR 2026) записывают потолок как неравенство. Для корпуса из n документов, top-k запросов и зазора score γ размерность эмбеддинга d должна удовлетворять условию $d \\geq \\frac{\\log \\binom{n}{k}}{\\log(1 + 1/\\gamma)}$. Ниже этой границы часть top-k комбинаций просто нельзя представить в векторном пространстве.
...
Улучшения есть. Все настоящие, и ни одно не меняет базовый цикл: эмбеддинг, поиск, чтение, генерация. Каждый запрос проходит цикл заново.

От RAG отказываться не надо. RAG действительно правильный первый ответ на вопрос "как прицепить LLM к своим данным". Проблема в другом: в индустрии к шаблону (проэмбеддить всё, искать на каждый запрос, подклеивать чанки в промпт) относятся как к платформе, хотя это прототип, разваливающийся на реальных размерах корпусов и реальной структуре релевантности.
...
Самая портативная версия: плагин. Одно Agent Skills определение, которое запускается на любом крупном LLM CLI без этапа сборки. Интересная деталь: горячий кэш. Маленький markdown-файл в несколько сотен слов лежит поверх вики и хранит свежий контекст текущей работы. В начале сессии он читается, в конце обновляется, и непрерывность держится именно на этом промежутке. Без него каждая новая сессия начинается с амнезии. Кэш протухает, если работа переключается между контекстами без обновления в конце сессии, и ничто в системе этого не замечает.
...
Системы агентной памяти (Letta, Mem0, Zep) запоминают по-человечески: прогоняют LLM по входящему сообщению, извлекают факты, кладут их в гибридное хранилище. Zep интереснее остальных одной деталью -- каждое ребро в графе несёт временну&#769;ю метку, и старые убеждения остаются с явным сроком годности вместо молчаливого перезатирания. Лучший публичный результат -- 66,9% точности на LOCOMO, с шестиочковым отставанием от полного контекста. Вроде бы работает. Но артефакт -- база данных, которую человек не откроет.

Граф-RAG системы подходят с другой стороны. GraphRAG от Microsoft строит граф сущностей, кластеризует его и заранее пишет саммари для каждого кластера. Результат впечатляет на multi-hop запросах, но индексация миллиона токенов стоит $20-50. Для корпуса, который обновляется каждый день, это не бюджет, а подписка.
