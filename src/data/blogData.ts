...
Статья в пяти частях. Часть 1 проходит по местам, где плоский RAG ломается, со ссылками на исследования. Часть 2 -- архитектурная альтернатива, LLM Wiki Karpathy, и почему она переформулирует пайплайн как компиляцию, а не как поиск. Часть 3 -- другие системы, делающие тот же манёвр в разных формах: системы агентной памяти, граф-RAG, HippoRAG. Часть 4 -- где вики оказывается неправильным ответом. Часть 5 -- что ломается первым, когда её действительно строишь, из опыта сборки 197-коммитного хранилища для команды AI-инфраструктуры.
...
От RAG отказываться не надо. RAG действительно правильный первый ответ на вопрос «как прицепить LLM к своим данным». Для прототипа, для PoC, для первой интеграции он работает, стоит недорого и показывает форму задачи. Проблема начинается при масштабировании. Когда к циклу эмбеддинг-поиск-чтение-генерация относятся как к архитектуре -- как к тому, на чём можно держать продукт, -- он ломается именно там. Платит полную стоимость извлечения на каждый вопрос и упирается в тот же потолок, где упирается геометрия.
