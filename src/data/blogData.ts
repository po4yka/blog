...
On carrier networks this asymmetry has long been the norm. A mobile carrier doesn't stop at routing: it fingerprints TLS and QUIC handshakes, throttles per connection, clamps the MTU, and strips ECN; an in-path box kills a connection that a home router would pass without a second look. So one destination is dead, the next one is fine, and any global "turn it on everywhere" switch is wrong for at least one of them.
...
The figure is the decision logic; here is the probe running it, three times against the repo's local-network-fixture, which stands in for the middlebox on loopback:
...
Ten core strategies ship built in, registered through \`linkme\` distributed slices, so adding one means a single entry and no central \`match\` statement. The names are utilitarian: \`split\` fragments a segment, \`seq_overlap\` lays data over sequence numbers; the rest are in the same vein. Two more, \`synack\` and \`synack_split\`, are registered next to them as \`Unimplemented\` placeholders; SYN-ACK injection runs on a separate path, through the TUN ingress interceptor. The registry tries strategies in registration order and takes the first whose \`plan\` builds. If applying it fails, an \`OnFail\` policy decides: roll back to the next one, fall back to plain traffic (the floor when nothing works), or drop the connection.
...
With no relay configured, the tunnel doesn't change the exit IP: traffic still leaves the device directly. On the way out the packets are only rewritten, so the destination sees the real address and a slightly stranger-looking handshake.
...
До последнего вердикта добраться намеренно трудно: нужно, чтобы ни один IPv4-адрес из DoH и ни один запасной IPv6 не ответили на SYN, и чтобы это подтвердило второе независимое соединение. Пока подтверждения нет, раннер сидит в \`PendingSecondFlow\` и вердикт не выносит. Ложное срабатывание здесь загнало бы соединение на ненужный relay, поэтому раннер ждёт доказательств. Когда вердикт всё же выносится, раннер ставит \`arm_gate = OwnedStackOnly\`: TLS-приёмы пропускаются, и движок сразу переходит к собственному стеку, то есть к relay. Переписывать пакеты бессмысленно, если по адресу никого нет.
...
Десять стратегий ядра встроены и регистрируются через distributed slices из \`linkme\`, так что добавить новую — одна запись и никакого центрального \`match\`. Имена утилитарные: \`split\` дробит сегмент, \`seq_overlap\` накладывает данные по номерам последовательности; остальные в том же духе. Ещё две, \`synack\` и \`synack_split\`, зарегистрированы рядом с ними заглушками \`Unimplemented\`: вброс SYN-ACK идёт другим путём, через перехватчик на входе TUN. Реестр пробует стратегии по порядку регистрации и берёт первую, у которой \`plan\` собрал шаги. Если применить не вышло, политика \`OnFail\` решает: откатиться к следующей, перейти на обычный трафик (последний вариант, если не сработало ничего) или сбросить соединение.
...
Без настроенного relay-сервера туннель не меняет внешний IP: трафик всё так же уходит с устройства напрямую. Пакеты на выходе лишь переписываются, так что адресат видит реальный адрес и рукопожатие, которое выглядит чуть необычнее.
...
<BlogFigure
  ...
  alt="Log-log chart plotting usable corpus size against embedding dimension. Weller et al. empirical fit (coral line) passes through two labeled anchor points: d=768 with n≈1.7M documents (open-weights default) and d=4096 with n≈250M (current frontier). Shaded unreachable region above the line. Formula at top: d ≥ log C(n,k) / log(1+1/γ). Benchmark strip reports LIMIT (50K docs, 1K queries) with E5-Mistral, GritLM, Qwen3 below 20% Recall@100 at d=4096 while BM25 reaches 93.6% Recall@100; GTE-ModernColBERT (vector-per-token) recovers 54.8% Recall@100 on the same set."
...
> Instead of just retrieving from raw documents at query time, the LLM incrementally builds and maintains a persistent wiki [...] This is the key difference: the wiki is a persistent, compounding artifact.
>
...
The vault behind this part serves an AI infrastructure team: I built its structure and automation, and the team works in it day to day. When you build a wiki-shaped knowledge base it fails in a specific order. I learned the rules for each failure by shipping the wrong fix first.
...
> Вместо того чтобы при каждом запросе искать по сырым документам, LLM инкрементально строит и поддерживает вики, которая сохраняется между запросами [...] В этом главное отличие: вики — долговременный артефакт, ценность которого накапливается.
>
> — Andrej Karpathy, гист LLM Wiki
...
Система построена на Bash-скриптах поверх хранилища Obsidian. В ней горстка агентов, набор навыков и Bash-адаптеры, которые из одного исходного дерева собирают конфигурации для четырёх LLM CLI; жёсткий контракт один — какие инструменты каждый агент может использовать. Каждый агент объявляет свои примитивы из фиксированного набора — одному, например, разрешено читать страницы и писать индекс, но запрещено трогать файл схемы. Объявит что-то за пределами списка — сборка упадёт. Эта конструкция почти ничего не перекладывает на ингест: агенты собирают по запросу, и работы примерно столько же, сколько без вики вообще. Слабое место, на мой взгляд, — сами платформы: одно хранилище на несколько LLM CLI держится только потому, что контракт достаточно узкий и скрывает различия между их интерфейсами.
...
Хранилище, о котором идёт речь в этой части, обслуживает команду AI-инфраструктуры: структуру и автоматизацию строил я, а команда пользуется им каждый день. Вики-хранилище ломается в определённой последовательности. Правила для каждой поломки я узнал, сначала выкатив неправильный фикс.
