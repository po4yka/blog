...
**Content pipeline (single-source):** Canonical sources are MDX files (`src/content/blog/`) and JSON files (`src/content/projects.json`, `src/content/experience.json`). Build-time generators produce static TypeScript data files (`blogData.ts`, `blogMeta.ts`, `projectsData.ts`, `experienceData.ts`) and `db/seed.sql`. `blogMeta.ts` is `blogData.ts` without post bodies: client islands must import from it (`blogPostsMeta`), never from `blogData.ts`, so full article text stays out of the browser bundle. `changelog.ts` is generated from the git history (feat/fix/perf conventional commits, one release per day, build = commit ordinal) and feeds `/changelog`; like `buildMeta.ts` it is refreshed only in CI or with `BLOG_REFRESH_BUILD_META=1`, and CI checks out with full history for it. Run `npm run generate:all` to regenerate all, or `npm run generate:blog` for blog only. Never edit generated data files manually.
...
The site is tuned for AI crawlers and agent tooling. Preserve these when making changes — they are load-bearing for citation and retrieval:

- **`public/robots.txt`** — explicit per-bot `Allow`/`Disallow` rules for GPTBot, OAI-SearchBot, ChatGPT-User, ClaudeBot, Claude-SearchBot, Claude-User, Google-Extended, PerplexityBot, CCBot, Meta-ExternalAgent, Applebot-Extended, Bytespider, Amazonbot, etc. Two sitemaps listed: `sitemap-index.xml` and `llms.txt`. Admin tree is disallowed for every bot.
- **`public/llms.txt`** — hand-authored discovery index (keep short). **`src/pages/llms-full.txt.ts`** — auto-generated full-content dump of every post + project + experience row, served at `/llms-full.txt`.
- **`src/pages/blog/[slug].md.ts`** and **`src/pages/blog/ru/[slug].md.ts`** — raw-markdown endpoints for coding agents and MCP fetchers. Every post is reachable at `/blog/{slug}.md` and `/blog/ru/{slug}.md`.
- **RSS feeds** — `src/pages/rss.xml.ts` (English) and `src/pages/rss.ru.xml.ts` (Russian) via `@astrojs/rss`. Linked from `<head>` with `hreflang`-aware `<link rel="alternate">`.
- **Sitemap** — generated by `@astrojs/sitemap`; no manual edits.
- **Structured data** — `src/components/JsonLd.astro` emits `Person` + `WebSite` sitewide and `BlogPosting` per post (with ISO `datePublished`/`dateModified`, `inLanguage`, `wordCount`, `mainEntityOfPage`). `src/components/ProjectsJsonLd.astro` emits an `ItemList` of `SoftwareApplication`/`MobileApplication` on `/projects`. Do not drop fields or revert to human-string dates.
- **`src/layouts/BaseHead.astro`** — owns canonical, `og:*` (including `og:locale` + `og:site_name`), `twitter:*`, RSS links, and self-referential `hreflang` + `x-default` for every post (not only translated ones). Per-post OG images at `/og/{lang}-{slug}.png` fall back to `/og-default.png` for non-blog pages.
- **Per-post OG images** — built by `scripts/generate-og-images.ts` (Satori + Resvg) into `public/og/` at build time; wired through `npm run generate:all`. Runtime rendering is not an option on Cloudflare Workers.
- **Semantic article markup** — `src/components/BlogPostIsland.tsx` wraps posts in `<article>` with a `<header>` containing `<h1>`, `<time dateTime={iso}>`, and tag list. Keep this shape; parsers depend on it.
- **Admin noindex** — admin layout emits `<meta name="robots" content="noindex,nofollow">`. Do not remove.
- **Static-first rule** — AI crawlers do not execute JavaScript. Any *content* must land in initial HTML; only *interactive chrome* should live inside hydrated islands.

When adding a new content surface (a new page type, collection, route), consider: does it need a JSON-LD node, an entry in `llms-full.txt`, an `.md` variant, and a sitemap entry?
...
- Build-time generators in `scripts/` — data files, OG images. Regenerate via `npm run generate:all`.
- MDX blog frontmatter must include ISO `publishedAt` (e.g. `"2026-04-01"`); `date` remains the human-facing display label (`"Apr 2026"`). Add `updatedAt` when editing an existing post.
